...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps
            ...
            .SelectMany(empDept => grades, (empDept, grade) => new { empDept, grade })
            .Where(x => x.empDept.emp.Sal >= x.grade.Losal && x.empDept.emp.Sal <= x.grade.Hisal)
            .Select(x => new
            {
                EName = x.empDept.emp.EName,
                DName = x.empDept.dept.DName,
                Grade = x.grade.Grade
            })
            .ToList();
